...
class Worker implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.out.printf("%s acquired the lock", name);
            System.out.printf("Thread: %s entered a critical session %n", name);
            System.out.printf("%d Threads waiting for the lock %n", lock.getQueueLength());
            System.out.printf("Thread %s is going to wait 2s %n", name);
            Thread.sleep(2000);
